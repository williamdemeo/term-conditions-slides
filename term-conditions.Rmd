...
<div align="left">**Problem Statement**
  <br>
...
+ **Theorem** (Kearnes *J Algebra* 1995) <br>
<a style="color:#e7ad52">$\mathbb{V}(\mathbf{A})$ has a diff term</a> $\Leftrightarrow$
<a style="color:#e7ad52">$\mathbb{V}(\mathbf{A})$ has a Taylor term and no type-2 tails</a>
...
+ **Proof Idea**
  ...
  <ul> 
    ...
    <li> Omitting 1's and 5's is poly-time decidable by Valeriote's subtype theorem.</li>
    <li> If there is a tail in $\mathbb{V}(\mathbf{A})$, then there is a tail <a style="color:#e7ad52">near the bottom</a>.</li>
  </ul>
  </small>
...
- Freese and Valeriote prove moreover: 
  <p>
...
- Let $\mathcal S$ be a finite set of finite, similar, idempotent algebras, 
  closed under taking subalgebras, such that $\mathcal V = \mathbb{V}(\mathcal S)$ omits 1.

- If there is a finite algebra in $\mathcal V$ having a type-2 minimal 
  set with a tail, then, by standard tct, such an algebra occurs as a subalgebra 
  of a product of members of $\mathcal S$. 

---

- Suppose a finite algebra $\mathbf{B}$ in $\mathcal V$ has a prime 
  quotient of type~2 whose minimal sets have non-empty tails.

- We show $\exists$ a 3-generated subalgebra of the
  product of two members of $\mathcal S$ with this property.

- Since $\mathcal S$ is closed under taking subalgebras,
  we may assume $\mathbf{B}$ is a subdirect
...
- Choose $n$ minimal such that for some $\mathbf{A}_0$,
  ...
  with a prime quotient of type~2 whose minimal sets have
  non-empty tails.
  
- Under the assumption that $n > 1$ we prove that $n = 2$.
